%%%%% Script to run Tax-Loss Harvesting on the iShares Canadian Universe Bond Index ETF  
%%%%% (Ticker XBB).

%% Use Yahoo Finance to get daily data as far back as possible to Jan/1st/2000
XBB_Data = getYahooDailyData('XBB.TO', '1/1/2000', '10/31/2014', 'mm/dd/yyyy');
XBB_Dates = table2array(XBB_Data.XBB0x2ETO(:,1));
XBB_Prices = table2array(XBB_Data.XBB0x2ETO(:,5));

% plot(XBB_Dates,XBB_Prices);
% dateaxis('x');
clear XBB_Data;

%% Calculate daily log-returns
XBB_Returns = log(XBB_Prices(2:end) ./ XBB_Prices(1:(end-1)));
XBB_Dates = XBB_Dates(2:end);
...
for i = 1:length(thresholds)
    TLHOutput = TLH(XBB_Returns,initialDeposit,initialWeights,thresholds(i),taxRate,taxRateEnd);
...
TLHOutputOptimal = TLH(XBB_Returns,initialDeposit,initialWeights,optimalthreshold,taxRate,taxRateEnd);

%% Plot cumulative-return and date at which harvesting occured
EFA_CumReturns = exp(cumsum(XBB_Returns));
plot(XBB_Dates(TLHOutputOptimal.HarvestedDates{1}),EFA_CumReturns(TLHOutputOptimal.HarvestedDates{1}),'ro','markerfacecolor','r');
hold on;
plot(XBB_Dates,EFA_CumReturns);
title('XBB Cumulative Returns');
axis tight
dateaxis('x',10);

datevec(XBB_Dates([1 end]))
...
HMMModel = MS_Regress_Fit(XBB_Returns,ones(length(XBB_Returns),1),2,[1 1]);
