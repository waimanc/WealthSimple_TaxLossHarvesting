%%%%% Script to run Tax-Loss Harvesting on the iShares 1-5 Year Laddered Corporate Bond Index ETF 
%%%%% (Ticker CBO).
...
CBO_Dates = table2array(CBO_Data.CBO0x2ETO(:,1));
CBO_Prices = table2array(CBO_Data.CBO0x2ETO(:,5));

clear CBO_Data;

%% Calculate daily log-returns
CBO_Returns = log(CBO_Prices(2:end) ./ CBO_Prices(1:(end-1)));
CBO_Dates = CBO_Dates(2:end);
...
for i = 1:length(thresholds)
    TLHOutput = TLH(CBO_Returns,initialDeposit,initialWeights,thresholds(i),taxRate,taxRateEnd);
...
TLHOutputOptimal = TLH(CBO_Returns,initialDeposit,initialWeights,optimalthreshold,taxRate,taxRateEnd);

%% Plot cumulative-return and date at which harvesting occured
CBO_CumReturns = exp(cumsum(CBO_Returns));
plot(CBO_Dates(TLHOutputOptimal.HarvestedDates{1}),CBO_CumReturns(TLHOutputOptimal.HarvestedDates{1}),'ro','markerfacecolor','r');
hold on;
plot(CBO_Dates,CBO_CumReturns);
title('CBO Cumulative Returns');
axis tight
dateaxis('x',10);

datevec(CBO_Dates([1 end]))
...
HMMModel = MS_Regress_Fit(CBO_Returns,ones(length(CBO_Returns),1),2,[1 1]);
